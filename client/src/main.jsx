...
function App() {
  ...
  const proceed = () => {
    ...
    socket.emit('hold_seats', { seatIds: selected })
  }
  const pay = () => {
    socket.emit('complete_payment', { bookingId, transactionId, result: paymentResult })
  }
  ...
  return <main className="app-shell">
    <header className="topbar">
      ...
      <div className={`connection ${connection ? 'online' : ''}`}><Wifi size={15} /> {connection ? 'Live sync' : 'Connecting...'}</div>
    </header>
    <section className="layout">
      <div className="primary-panel">
        {screen === 'booking' && <BookingView seats={seats} selected={selected} toggleSeat={toggleSeat} proceed={proceed} total={total} />}
        {screen === 'payment' && <PaymentView paymentResult={paymentResult} setPaymentResult={setPaymentResult} pay={pay} />}
        {screen === 'success' && <ResultView type="success" selected={selected} total={total} bookingId={bookingId} reset={reset} />}
      ...
      <aside className="inspector">
        ...
        <div className="legend compact"><span><i className="available" /> Available</span><span><i className="held" /> Held</span><span><i className="booked" /> Booked</span></div>
      </aside>
    </section>
